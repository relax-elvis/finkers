...
$(document).ready(function() {
	...
	$('.f_banner .block-form').eq(0).find('form').submit(function(e) {
		...
		$('span.result').text(formatter.format(couta_inicial));
        

        $("[name='prestamo-range']").attr("max",prestamo_maximo);

        $(".range-sum.prestamo-range span").text(prestamo_maximo);
        $(".range-sum.plazo-range span").text(plazo);
        $(".range-sum.interes-range span").text(interes*100);
